...
#define brightness 2  // Sets the brightness level
...
void loop() {
  ...
  if (new_data == true) {
    ...
    unsigned int it = ceil(iterator / 2.0);  // Use ceil function to round up the result of division
    ...
    if (iterator == 0) {
      ...
      // Loop through the messageMap using the iterator
      for (auto it = messageMap.begin(); it != messageMap.end(); ++it) {
        // Access the current key-value pair using it->first and it->second
        // uint8_t *macAddr = it->first;     // Get the MAC address
        struct_message msg = it->second;  // Get the message

        if (iterator % 2 == 1) {
          // Display the customMessage string
